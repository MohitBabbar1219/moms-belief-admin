...
class ClinicalExpertCards extends Component {
  constructor(props) {
    super(props);
  ...
  updateHomeElements = () => {
    axios.get('/api/aboutUs/clinicalExperts').then(testimonials => {
      console.log(testimonials.data);
      this.setState({clinicalExperts: testimonials.data.data});
    });
  };
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        <button className="btn btn-primary m-4" onClick={() => this.toggleNew("clinicalExperts")}>Add new
          clinical consultant
        </button>
        <Row>
          {testimonialSection}
...
export default ClinicalExpertCards;
