...
class ProgramAssistanceCards extends Component {
  constructor(props) {
    super(props);
  ...
  updateHomeElements = () => {
    axios.get('/api/homeBasedProgram/programAssistances').then(testimonials => {
      console.log(testimonials.data);
      this.setState({programAssistances: testimonials.data.data});
    });
  };
  ...
  postData = (e = null) => {
    ...
    axios.post(`/api/homeBasedProgram/${this.state.elementBeingEdited.type}`, formData, config)
      .then((response) => {
  ...
  delete = (type, id) => {
    ...
    axios.delete(`/api/homeBasedProgram/${type}/${id}`, config)
      .then((response) => {
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        <button className="btn btn-primary m-4" onClick={() => this.toggleNew("programAssistances")}>Add new
          program assistance
        </button>
        <Row>
          {testimonialSection}
...
export default ProgramAssistanceCards;
