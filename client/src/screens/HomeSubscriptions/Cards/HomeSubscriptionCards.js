...
class HomeSubscriptionCards extends Component {
  constructor(props) {
    super(props);
  ...
  updateHomeElements = () => {
    axios.get('/api/homeBasedProgram/subscriptions').then(testimonials => {
      console.log(testimonials.data);
      this.setState({subscriptions: testimonials.data.data});
    });
  };
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        <button className="btn btn-primary m-4" onClick={() => this.toggleNew("subscriptions")}>Add new
          subscription
        </button>
        <Row>
          {testimonialSection}
