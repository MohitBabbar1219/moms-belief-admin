...
class SchoolCards extends Component {
  ...
  updateHomeElements = () => {
    axios.get('/api/schools/schools').then(testimonials => {
      console.log(testimonials.data);
      this.setState({schools: testimonials.data.data});
    });
  };
  ...
  render() {
    let testimonialSection = <Aux>
      {this.state.schools.map((clinicalConsultant, index) => {
        return this.state.editableElement === clinicalConsultant ?
          ...
          <SchoolCard edit={() => this.edit("schools", index)}
                      key={clinicalConsultant._id}
                      delete={() => this.delete("schools", clinicalConsultant._id)}
                      school={clinicalConsultant}/>
      })}
    </Aux>;
    return (
      <div className="animated fadeIn">
        <button className="btn btn-primary m-4" onClick={() => this.toggleNew("schools")}>Add new
          school
        </button>
        <Row>
          {testimonialSection}
